...
class Wind implements \wf\template\EngineInterface
{
    ...
    protected function compile($tplFile, $compiledFile)
    {
        ...
        $template = $this->clearCommentTag($template);
                                ...
                                $template = preg_replace_callback("/\\{for\\s+(.*?)\\}/is", function($m){
                                    ...
                                    if(preg_match_all("/\\{loop\\s+?(\\S+?)\\s+?(\\S+?)\\}/s", $template, $matches)) {
                                        ...
                                        foreach ($matches[0] as $k => $mat) {
                                            ...
                                            ?>";
                                            $replace[$k] = $this->quote($replaceStr);
                                        }
                                    ...
                                    $template = preg_replace_callback("/\\{\\{\\s*([@a-z_\\$\\\\].+?)\\s*\\}\\}/i", function($m){
                                        return $this->quote("<?php echo @{$m[1]};?>");
                                    }, $template);
    ...
    private function parseExtend($template)
    {
        ...
        $masterPath  = $this->getTplPath($match[1] . '.html');
        $masterTpl   = @file_get_contents($masterPath);
        $masterTpl   = $this->clearCommentTag($masterTpl);
     ...
     * @return unknown
     */
    private function clearCommentTag($template)
    {
        // 去掉<!--{}-->的<!-- -->
    ...
    protected function subTpl($m)
    {
        ...
        if ($this->cfg['compileMerge']) {
            ...
            $content = $this->clearCommentTag($content);
     ...
     */
    protected function getRealTplPath($file) {
